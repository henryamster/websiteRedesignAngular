 ...
 Otherwise, return true. */
  private asTrueWhenExpirationDateHasNotPassed(): boolean {
    ...
    if (this.blogPost.expiryDate instanceof Date){
      return this.blogPost.expiryDate.getTime() > this.now;
      // return (this.blogPost.expiryDateas; Date; ) () as new Date;
    }
    if (typeof this.blogPost.expiryDate =='string'){
      return new Date(this.blogPost.expiryDate).getTime() > this.now;
    }
// console.log((this["blogPost"]["expiryDate"] as Timestamp).seconds*1000,new Date().getTime()*1000)
    return (this.blogPost.expiryDate as Timestamp).seconds  > new Date().getTime() / 1000;
  }
